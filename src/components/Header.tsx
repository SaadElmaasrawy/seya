    ...
    <header className="relative w-[90%] flex items-center justify-between whitespace-nowrap px-4 sm:px-6 py-4 z-50 m-auto">
      ...
      <div className="hidden md:flex flex-1 justify-end items-center gap-8">
        <nav className="flex items-center gap-9">
          <Link className="text-white hover:text-[#007BFF] text-sm font-medium leading-normal transition-colors" href="/#features">
            {t("Features")}
          </Link>
          <Link className="text-white hover:text-[#007BFF] text-sm font-medium leading-normal transition-colors" href="/#pricing">
            {t("Pricing")}
          </Link>
          <Link className="text-white hover:text-[#007BFF] text-sm font-medium leading-normal transition-colors" href="/capabilities">
            {t("Capabilities")}
          </Link>
        </nav>
        <div className="flex gap-2 items-center">
          <button
            ...
            className="text-white hover:text-[#007BFF] text-sm font-bold px-2 transition-colors"
          >
            {language === "en" ? "AR" : "EN"}
          </button>
          {!checkingAuth && (
            isAuthenticated ? (
              <Link href="/chat" className="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 px-4 bg-[#007BFF] text-white text-sm font-bold leading-normal tracking-[0.015em] hover:bg-blue-600 transition-colors">
      ...
      <div className="md:hidden flex items-center gap-4">
        <button
          onClick={toggleLanguage}
          className="text-white hover:text-[#007BFF] text-sm font-bold transition-colors"
        >
          {language === "en" ? "AR" : "EN"}
        </button>
        <button
      ...
      {isMenuOpen && (
        <>
          ...
          {/* Sidebar */}
          <div className={`fixed top-0 ${language === 'ar' ? 'left-0' : 'right-0'} h-full w-[280px] bg-[#1E1E24] md:hidden z-[9999] shadow-2xl animate-slide-in-${language === 'ar' ? 'left' : 'right'}`}>
